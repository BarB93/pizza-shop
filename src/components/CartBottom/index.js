...
const CartBottom = () => {
  const { totalCount, totalPrice, totalWeight } = useSelector(state => state.cart)

  return (
    <div className={styles.bottom}>
      <div className={styles.bottom__details}>
        <div className={styles.bottom__detailsLeft}>
          <span>Всего пицц: <b>{pricePrettify(totalCount)} шт.</b></span>
          <span>Общий вес: <b>{pricePrettify(totalWeight)} гр.</b></span>
        </div>
        <div className={styles.bottom__detailsRight}>
          <span>Сумма заказа: <b>{pricePrettify(totalPrice)} ₽</b></span>
        </div>
      </div>
